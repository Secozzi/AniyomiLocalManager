package xyz.secozzi.aniyomilocalmanager.domain.ktor.cache
...
// From https://github.com/vipulasri/kachetor
class PersistentCache(
    ...
    private suspend fun getOkioFileCache(): OkioFileKache {
        return OkioFileKache(
            directory = context.cacheDir
                .makeFolder(context, dirName, CreateMode.REUSE)!!
                .absolutePath
                .toPath(),
            maxSize = maxSize,
        ) {
    ...
    override suspend fun remove(url: Url, varyKeys: Map<String, String>) {
        val cache = getOkioFileCache()
        cache.getIfAvailable(url.toString())?.let { path ->
            fileSystem.read(path) {
                readCache(this).let { c ->
                    if (varyKeys.all { (key, value) -> c.varyKeys[key] == value }) {
