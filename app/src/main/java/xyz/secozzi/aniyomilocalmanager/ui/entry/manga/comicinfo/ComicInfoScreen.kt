...
class ComicInfoScreen(val path: String, val anilistId: Long?) : Screen() {
    @Composable
    override fun Content() {
        ...
        val screenModel = rememberScreenModel(tag = "manga") {
            DetailsScreenModel(path, anilistId, AnilistSearchType.MANGA, fileManager, xml, trackerIdRepository, anilistSearch, anilistPreferences)
        }
        ...
        if (result != null) {
            ...
            screenModel.updateAniList(MANGA_DIRECTORY_NAME, result.remoteId)
